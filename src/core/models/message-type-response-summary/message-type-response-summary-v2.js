...
export default class MessageTypeResponseSummary extends Root {
  ...
  _scheduleSendResponseMessage() {
    if (!this._sendResponseTimeout) {
      this._sendResponseTimeout = setTimeout(() => {
        if (this._currentResponseModel) this.sendResponseMessage();
      }, 100);
    }
  }
...
MessageTypeResponseSummary.mixins = Core.mixins.ResponseSummaryModel;
