 ...
 *      query.destroy();
 *
 * @class  Layer.Core.Query.IdentitiesQuery
...
export default class IdentitiesQuery extends Query {
  _fetchData(pageSize) {
    ...
    if (!this._nextDBFromId && getClient().dbManager) {
      getClient().dbManager.loadIdentities((identities) => {
